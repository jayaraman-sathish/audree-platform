...
// Vertical labeled add/edit form (one labeled field per line, fixed template fields per master
// type via renderField's fieldKind()) instead of a bare row of table-cell inputs. Defined at
// module scope (not inside Masters()) so its identity is stable across renders -- otherwise
// every keystroke would recreate the component and the focused input would lose focus.
function EditForm({ cols, values, renderField, onSet, onSave, onCancel, saving }) {
  return (
    <div className="mform">
      {cols.map((c) => (
        <div key={c} style={{ marginBottom: 10 }}>
          {renderField(c, values[c], (v) => onSet(c, v), values)}
        </div>
      ))}
      <div className="edit-row-actions" style={{ marginTop: 6 }}>
        <button className="btn primary" disabled={saving} onClick={onSave}>Save</button>
...
export default function Masters() {
  ...
  // Deactivate/reactivate is a status toggle, not a delete: the row is retained (and its full
  // version history preserved) and simply excluded from runtime/copilot reads while inactive.
  const toggleStatus = useMutation({
    mutationFn: ({ id, isActive }) => (isActive
      ? api.patch(`/api/v1/masters/${active}/rows/${id}/status`, { is_active: true })
      : api.delete(`/api/v1/masters/${active}/rows/${id}`)),
  ...
  const history = useQuery({
    queryKey: ["masterRowHistory", active, historyRowId],
    queryFn: () => api.get(`/api/v1/masters/${active}/rows/${historyRowId}/history`).then((r) => r.data),
    enabled: historyRowId != null,
  });
  ...
  function diffKeys(oldData, newData) {
    ...
    return [...keys].filter((k) => String((oldData || {})[k] ?? "") !== String((newData || {})[k] ?? ""));
  }
    ...
    <div>
      ...
      {m && (
        <div className="card tbl-wrap">
          ...
          {m.note && <p className="hint" style={{ marginBottom: 12 }}>{m.note}</p>}
          ...
          {editing && editing.id === null && (
            <EditForm
              ...
              onSet={(c, v) => setEditing({ ...editing, values: { ...editing.values, [c]: v } })}
              onSave={() => save.mutate({ id: null, data: editing.values })}
          ...
          <table>
            ...
            <tbody>
              {m.rows.map((row) => {
                ...
                return (
                  <Fragment key={row.id}>
                    <tr className={isRowActive ? "" : "row-inactive"}>
                      ...
                      {isAdmin && (
                        <td style={{ whiteSpace: "nowrap" }}>
                          ...
                          <button
                            className={isRowActive ? "btn danger" : "btn"}
                            title={isRowActive ? "Deactivate (status toggle, row is retained)" : "Reactivate"}
                            onClick={() => toggleStatus.mutate({ id: row.id, isActive: !isRowActive })}
                          >
                            {isRowActive ? "Deactivate" : "Reactivate"}
                          </button>{" "}
                          <button className="btn" onClick={() => setHistoryRowId(historyRowId === row.id ? null : row.id)}>
                    ...
                    {editing && editing.id === row.id && (
                      <tr key={`${row.id}-edit`}>
                        <td colSpan={m.cols.length + (isAdmin ? 1 : 0)}>
                          <EditForm
                            cols={m.cols}
                            values={editing.values}
                            renderField={renderField}
                            onSet={(c, v) => setEditing({ ...editing, values: { ...editing.values, [c]: v } })}
                            onSave={() => save.mutate({ id: row.id, data: editing.values })}
                            onCancel={() => setEditing(null)}
                            saving={save.isPending}
                          />
                        </td>
                    ...
                    {historyRowId === row.id && (
                      <tr key={`${row.id}-history`}>
                        <td colSpan={m.cols.length + (isAdmin ? 1 : 0)}>
                          <div className="mform">
                            <h3 style={{ marginBottom: 8 }}>Version history — {row.data[m.cols[0]]}</h3>
                            {history.isLoading && <p className="hint">Loading…</p>}
                            {history.data && history.data.length === 0 && <p className="hint">No changes recorded yet.</p>}
                            {history.data && history.data.length > 0 && (
                              <table className="vh-table">
                                <thead><tr><th>Version</th><th>Action</th><th>Changed fields</th><th>Actor</th><th>Note</th><th>Time</th></tr></thead>
                                <tbody>
                                  {history.data.map((h) => (
                                    <tr key={h.id}>
                                      <td className="mono">{h.version_label}</td>
                                      <td>{h.action}</td>
                                      <td>
                                        {diffKeys(h.old_data, h.new_data).map((k) => (
                                          <div key={k}>
                                            <b>{k}</b>: <span className="mono">{String((h.old_data || {})[k] ?? "—")}</span> → <span className="mono">{String((h.new_data || {})[k] ?? "—")}</span>
                                          </div>
                                        ))}
                                      </td>
                                      <td>{h.actor}</td>
          ...
          {m.json_example && (
            <>
              <h3 style={{ marginTop: 16, marginBottom: 8 }}>Example payload</h3>
              <div className="jsonbox">{m.json_example}</div>
            </>
          )}
