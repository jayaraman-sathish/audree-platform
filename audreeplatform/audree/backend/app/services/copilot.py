...
def _validate_commitment_fields(e: dict, sim_products: dict):
    """Real input validation for the INT-PPIC-001 commitment-check path,
    required by the client: reject/ask-clarification on nonsense
    quantities, past required_dates, and product strings that don't
    resolve to a known SIM_PRODUCTS entry -- instead of silently running
    the commitment math on bad data. Returns (field, label, question) if
    invalid, else None. This ADDS checks; it never changes the decision
    math in run_commitment_check() itself."""
    product = e.get("product")
    if product is not None and product not in sim_products:
        return ("product", "Product",
                f"'{product}' doesn't match a known product in the catalog. Which product should I check "
                f"(e.g. Amoxicillin 500 mg, Paracetamol 650 mg, Azithromycin 250 mg)?")

    qty = e.get("qty_m")
    if qty is not None and qty <= 0:
        return ("qty_m", "Quantity",
                f"'{qty} million' isn't a valid quantity to commit. Please provide a positive quantity.")

    required_date = e.get("date")
    if required_date is not None and required_date < P_TODAY:
        return ("date", "Required Date",
                f"The required date {_fmt_date(required_date)} is in the past (today is {_fmt_date(P_TODAY)}). "
                f"Please provide a valid future delivery date.")
...
def run_commitment_check(db: Session, req_id: str, cor_id: str, entities: dict) -> dict:
    ...
    for mt in p["materials"]:
        mat_inv = inv[mt["m"]]  # local fallback loaded up-front (used only if the dispatcher degrades)
        try:
            dispatched = dispatch(db, "GET_WMS_STOCK", {"material": mt["m"]}, req_id, cor_id)
            wms = dispatched["data"]
            mat_inv = {"stock": wms["stock"], "reserved": wms["reserved"], "uom": wms["uom"],
                       "open_po": wms["open_po"],
                       "po_eta": dt.date.fromisoformat(wms["po_eta"]) if wms.get("po_eta") else None}
        except ToolDispatchError as exc:
            # Degrade gracefully: keep going with the figures already loaded
            # via _load_sim rather than failing the whole request -- mirrors
            # the app's existing "partial results allowed; missing data
            # clearly marked" pattern (Standard Agent Output Format note).
            add_audit(db, req_id, cor_id, "Order Commitment Check", "DECISION",
                      f"GET_WMS_STOCK degraded for {mt['m']}: {exc} -- using cached WMS figures", "ERR")
    ...
    line_free = lines[p["line"]]  # local fallback (used only if the dispatcher degrades)
    try:
        cap = dispatch(db, "SAP_PP_CAPACITY_CHECK", {"line": p["line"]}, req_id, cor_id)
        cap_data = cap["data"]
        if cap_data.get("free_from"):
            line_free = dt.date.fromisoformat(cap_data["free_from"])
    except ToolDispatchError as exc:
        add_audit(db, req_id, cor_id, "Order Commitment Check", "DECISION",
                  f"SAP_PP_CAPACITY_CHECK degraded for {p['line']}: {exc} -- using cached capacity figures",
                  "ERR")
...
def handle_message(db: Session, text: str, session_id: str) -> dict:
    ...
    if not code:
        add_audit(db, req_id, cor_id, "Enterprise Copilot", "INTENT",
                  "Unknown intent (confidence < 0.50) — handing off to LLM orchestrator for open-ended reasoning",
                  "OK")
        llm_result = llm_orchestrator.handle_open_query(db, text, req_id, cor_id, session_context=mem)
        if llm_result.get("status") == "ok":
            return {"type": "llm_answer", "request_id": req_id, "correlation_id": cor_id,
                    "tools_called": llm_result.get("tools_called", []), "message": llm_result["answer"]}

        # LLM couldn't help either (not configured, or an error) -- honest,
        # non-crashing fallback with the supported-question examples.
        return {"type": "unknown", "request_id": req_id, "correlation_id": cor_id,
                "message": llm_result["answer"] + "\n\nI couldn't map your question to a configured business "
                           "intent either. I can help with: Order Commitment, Production Feasibility, Material "
                           "Availability, Procurement, Replenishment, Batch Release, Executive KPIs."}
...
def _proceed(db: Session, req_id: str, cor_id: str, code: str, intent_name: str, e: dict, mem: dict,
    ...
    products, _, _, _ = _load_sim(db)
    invalid = _validate_commitment_fields(e, products)
    if invalid:
        field_name, label, question = invalid
        add_audit(db, req_id, cor_id, "Enterprise Copilot", "CLARIFY",
                  f"clarification_required · invalid value: {field_name} (INT-422-001)", "HUMAN")
        return {"type": "clarify", "request_id": req_id, "correlation_id": cor_id, "intent_code": code,
                "intent_name": intent_name, "routing": routing_note, "field": field_name, "label": label,
                "question": question, "entities": _jsonify(e), "recalled": recalled,
                "message": f"{question}"}
