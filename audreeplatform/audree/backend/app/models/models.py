...
class MasterRowHistory(Base):
    """Per-row version history for Configuration Masters, derived alongside
    every create/update/deactivate so the UI can render a diff + timestamp +
    actor without reconstructing it from free-text audit_log entries.
    Additive table -- does not touch existing master_row/master_version data."""
    __tablename__ = "master_row_history"
    __table_args__ = {"schema": "cfg"}

    id = Column(Integer, primary_key=True)
    master_id = Column(String(32), ForeignKey("cfg.master_version.master_id"), nullable=False, index=True)
    row_id = Column(Integer, ForeignKey("cfg.master_row.id"), nullable=False, index=True)
    version_label = Column(String(16))  # master version at the time of this change, e.g. v1.3
    action = Column(String(16), nullable=False)  # created|updated|deactivated|reactivated
    old_data = Column(JSONB, nullable=True)
    new_data = Column(JSONB, nullable=True)
    actor = Column(String(120))
    note = Column(Text)  # e.g. "modified after publish" vs "modified as unpublished draft"
    created_at = Column(DateTime, default=UTCNOW)
...
class AgentToolExecution(Base):
    """rt.agent_tool_execution -- real dispatch log for every invocation made
    through app.services.tool_dispatcher.dispatch(). Mirrors the Tool Registry
    Master's own note text ("every invocation validated (RBAC), timed out,
    retried and logged"): one row per attempt-set (i.e. per dispatch() call,
    including any internal retries), independent of AuditLog (which records
    business-decision events, not tool-level plumbing)."""
    __tablename__ = "agent_tool_execution"
    __table_args__ = {"schema": "rt"}

    id = Column(Integer, primary_key=True)
    tool_code = Column(String(64), index=True, nullable=False)
    request_id = Column(String(32), index=True)
    correlation_id = Column(String(32), index=True)
    status = Column(String(16), nullable=False)  # success|failed|timeout
