...
cp .env.example .env   # edit DATABASE_URL / JWT_SECRET / ANTHROPIC_API_KEY if needed
...
## LLM orchestration layer (open-ended / executive questions)

Audree has two Copilot reasoning modes (see `ARCHITECTURE.md` for the full
design): the existing deterministic Intent Engine for the 9 seeded plant
scenarios, and a real LLM orchestrator (`app/services/llm_orchestrator.py`,
using the Anthropic API / Claude with tool use) for open-ended, executive,
or cross-system questions that don't match a known intent.

To enable it, set `ANTHROPIC_API_KEY` as an environment variable (add it as
a Render environment variable in production, or in `backend/.env` locally).
**Without it, the app still runs fully** — the fixed Intent Engine keeps
working exactly as before, and the LLM layer responds with a clear "LLM
reasoning is not configured — set ANTHROPIC_API_KEY" message instead of
erroring, for any question that falls through to it.
